...
/** note for hmac:HMAC是密钥Option的哈希运算消息认证码（Hash-based Message Authentication Code）,HMAC运算利用哈希算法，以一个密钥和一个消息为输入，生成一个消息摘要作为输出。*/
...
#pragma mark --❤️-- aes256 (AES-256 加密Option)
...
#pragma mark --❤️-- encode and decode (编码或者解码Option)
...
+ (NSData *)azm_dataWithHexString:(NSString *)hexString;
+ (NSData *)azm_dataWithBase64EncodedString:(NSString *)base64EncodedString;

#pragma mark --❤️-- comperss and decompress(数据压缩或者解压缩Option)
